...
void ScoreView::moveControlCursor(const Fraction& tick)
{
    ...
    if (seq->isPlaying()) {
        ...
        if (_controlCursorTimer.isValid()) {
            if (!_panSettings.advancedWeighting || _controlModifier == _panSettings.minContinuousModifier
                || _controlModifier == _panSettings.maxContinuousModifier) {
                addition = _controlCursorTimer.elapsed() * _controlModifier;
            } else {
                addition = _controlCursorTimer.elapsed() * _controlModifier * _panSettings.normalWeight
                           + _controlCursorTimer.elapsed() * (_playbackCursorDistanceTravelled / 500)
                           * _panSettings.smartWeight;
            }
        }
        ...
        { { "next-chord",
            ...
            "prev-measure" }, [](ScoreView* cv, const QByteArray& cmd) {
                ...
                if (el && (el->isTextBase())) {
                    ...
                    if (cmd == "prev-chord") {
                        el->undoChangeProperty(Pid::OFFSET, el->offset() - QPointF(
                                                   MScore::nudgeStep* el->spatium(), 0.0), pf);
                    } else if (cmd == "next-chord") {
                        el->undoChangeProperty(Pid::OFFSET, el->offset() + QPointF(
                                                   MScore::nudgeStep* el->spatium(), 0.0), pf);
                    } else if (cmd == "prev-measure") {
                        el->undoChangeProperty(Pid::OFFSET,
                                               el->offset() - QPointF(MScore::nudgeStep10* el->spatium(), 0.0), pf);
...
void ScoreView::midiNoteReceived(int pitch, bool chord, int velocity)
{
    ...
    if (!chord && velocity && !realtimeTimer->isActive() && score()->usingNoteEntryMethod(
            NoteEntryMethod::REALTIME_AUTO)) {
...
{
    InputState& is = score()->inputState();
    bool realtime = is.usingNoteEntryMethod(NoteEntryMethod::REALTIME_AUTO) || is.usingNoteEntryMethod(
        NoteEntryMethod::REALTIME_MANUAL);
