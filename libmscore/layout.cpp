            ...
            if (separation == 1) {
                ...
                for (int i = 0, count = overlapNotes.size(); i < count; ++i) {
                    ...
                    case 0:
                        ...
                        nHeadType
                            = (n->headType()
                               == NoteHead::Type::HEAD_AUTO) ? n->chord()->durationType().headType() : n->headType();
                        pHeadType
                            = (p->headType()
                               == NoteHead::Type::HEAD_AUTO) ? p->chord()->durationType().headType() : p->headType();
                        ...
                        if (n->headGroup() != p->headGroup() || n->tpc() != p->tpc() || n->mirror() || p->mirror()
                            ...
                            if ((nchord->dots() != pchord->dots() || !nchord->stem() || !pchord->stem()
                                 || nHeadType != pHeadType || n->small() || p->small())
                                && ((n->headType() == NoteHead::Type::HEAD_AUTO
                                     && p->headType() == NoteHead::Type::HEAD_AUTO) || nHeadType != pHeadType)
                                && (n->visible() == p->visible())) {
                                shareHeads = false;
                            }
                        }
                ...
                if (shareHeads) {
                    ...
                    if (bottomUpNote->chord()->stem()) {
                        clearRight = bottomUpNote->chord()->stem()->lineWidth()
                                     + qMax(maxDownWidth - maxUpWidth, 0.0) + 0.3 * sp;
                    } else {
...
static qreal layoutAccidental(AcEl* me, AcEl* above, AcEl* below, qreal colOffset, QVector<Note*>& leftNotes, qreal pnd,
                              qreal pd, qreal sp)
{
    qreal lx = colOffset;
        ...
        for (int i = 0; i < tracks; ++i) {
            ...
            for (Note* n : c->notes()) {
                ...
                if (tie && !tie->endNote()) {
                    ...
                    if (nnote == 0) {
                        if (!silent) {
                            qDebug("next note at %d track %d for tie not found (version %d)",
                                   s->tick().ticks(), i, _mscVersion);
                            delete tie;
                            n->setTieFor(0);
                        }
                    } else {
...
void LayoutContext::getNextPage()
{
    ...
    if (curPage) {
        ...
        if (MScore::verticalOrientation()) {
            ...
            qreal gap = (curPage + score->pageNumberOffset())
            ...
            if (e) {
                ...
                if (!ds->element(staffIdx * VOICES)) {
                    ...
                    if (bd->barLineType() != bs->barLineType()) {
                        ...
                        undo(new ChangeProperty(bd, Pid::BARLINE_TYPE, QVariant::fromValue(bs->barLineType()),
                ...
                for (int t = track; t < endTrack; ++t) {
                    ...
                    if (cr->isChord()) {
                        ...
                        if (chord->tremolo() && chord == chord->tremolo()->chord2()) {
                            ...
                            if (stem1 && stem2) {
                                std::pair<qreal, qreal> extendedLen = extendedStemLenWithTwoNoteTremolo(
                                    chord->tremolo(),
                                    stem1->p2().y(),
                                    stem2->p2().y());
...
void Score::layoutLyrics(System* system)
{
    ...
    for (int staffIdx = system->firstVisibleStaff(); staffIdx < nstaves();
        ...
        for (Element* e : s->elist()) {
            ...
            if (e->isChord()) {
                ...
                for (Fingering* f : fingerings) {
                    ...
                    if (f->addToSkyline()) {
                        Note* n = f->note();
                        QRectF r = f->bbox().translated(
                            f->pos() + n->pos() + n->chord()->pos() + s->pos() + s->measure()->pos());
